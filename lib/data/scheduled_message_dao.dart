...
class ScheduledMessageDao {
  static const String SCHEDULED_MESSAGE_STORE_NAME = 'scheduled_messages';

  // A Store with int keys and Map<String, dynamic> values.
  // This Store acts like a persistent map, values of which are ScheduledMessage objects converted to Map
  final _messageStore = intMapStoreFactory.store(SCHEDULED_MESSAGE_STORE_NAME);
  ...
  Future<int> insert(Message message) async {
    int key = await _messageStore.add(await _db, message.toMap());
    return key;
  }
  ...
  Future<List<Message>> getAllSortedByTimestamp(int startIndex, int limit) async {
    ...
    // Making a List<ScheduledMessage> out of List<RecordSnapshot>
